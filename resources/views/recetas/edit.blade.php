    ...
    <form method="POST" class="needs-validation" action="{{ route('recetas.update', $receta->id) }}" novalidate>
        ...
        <div class="mb-3">
            ...
            <textarea 
                ...
                value="{{ $receta->instruction }}" 
                id="instructionInput" 
                rows="3" style="resize: none" 
                required 
            >{{ $receta->instruction }}</textarea>
        ...
        <div class="mb-3">
            ...
            <input 
                ...
                name="minutes" 
                class="form-control 
                @error('minutes') is-invalid @enderror" 
                id="minutesInput" 
                required 
                value="{{ $receta->minutes }}"
            >
...
@once
    @push('scripts')
        ...
        <script>var recetaIngredients = {{ Js::from($receta->ingredients) }}</script>
